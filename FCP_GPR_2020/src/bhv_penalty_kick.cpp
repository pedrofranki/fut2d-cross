...
Bhv_PenaltyKick::doDribble( PlayerAgent * agent )
{
    ...
    Vector2D drib_target = goal_c + Vector2D(ServerParam::i().goalHalfWidth()-sign(wm.ball().pos().x)*20,20);
...
Bhv_PenaltyKick::doGoalie( PlayerAgent* agent )
{
   ...
   if ( Bhv_BasicTackle( 0.8, 80.0 ).execute( agent ) )
    ...
    if( Bhv_BasicTackle( 0.3, 60.0 ).execute( agent ) )
    {
     ...
     if (self_min <= 3 || self_min < opp_min + 1 )
     {
         dlog.addText( Logger::TEAM,
                       __FILE__": intercept" );
     ...
     rcsc::Vector2D goalPos = Vector2D(rcsc::sign(wm.ball().pos().x)*52.5, 0.0);//54.5
     rcsc::Vector2D ballPos2 = wm.ball().inertiaPoint( opp_min );
     rcsc::Vector2D ball2goal = ballPos2 - goalPos;
     Vector2D target_point = goalPos + ball2goal*0.9;
...
#ifdef DEBUG_TARGET_POINT
    agent->debugClient().setTarget( move_pos );
#endif
#ifdef DEBUG_PENALTY_KICK
    agent->debugClient().addMessage( "Penalty Basic Move" );
#endif
